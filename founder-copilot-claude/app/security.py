...
def require_auth(authorization: str | None = Header(default=None)) -> AuthContext:
    ...
    if mode == "none":
        return AuthContext(user_id=None, mode=mode)
    if mode != "jwt":
        raise HTTPException(status_code=500, detail="AUTH_MODE must be none|jwt")
    ...
    return AuthContext(user_id=user_id, mode=mode)


def authorize_audit(authorization: str | None, x_api_key: str | None) -> AuthContext:
    admin_api_key = os.getenv("ADMIN_API_KEY", "").strip()
    if admin_api_key and x_api_key and x_api_key == admin_api_key:
        return AuthContext(user_id="admin_api_key", mode="admin_api_key")
